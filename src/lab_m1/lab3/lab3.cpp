...
void Lab3::Init()
{
    ...
    cx = corner.x + squareSide / 4;
    cy = corner.y + squareSide / 4;
    ...
    Mesh* rectangle = object2D::CreateRectangle("rectangle", corner, rectSide, rectHeight, glm::vec3(0, 0, 1));
    AddMeshToList(rectangle);

    Mesh* rectangle1 = object2D::CreateRectangle("rectangle1", corner, rectSide / 2, rectHeight / 2, glm::vec3(0, 0, 1));
    AddMeshToList(rectangle1);

    Mesh* square4 = object2D::CreateSquare("square4", corner, squareSide / 2, glm::vec3(1, 0, 0));
    AddMeshToList(square4);

    Mesh* square5 = object2D::CreateSquare("square5", corner, squareSide / 2, glm::vec3(0, 1, 0));
...
void Lab3::Update(float deltaTimeSeconds)
{
    ...
    translateX += reverse * deltaTimeSeconds * 200;

    if (translateX > 300 || translateX < 0)
        reverse *= -1;

    scaleX += deltaTimeSeconds * reverse;

    if (scaleX > 3 || scaleX < 0.5)
        reverse *= -1;
    ...
    modelMatrix = glm::mat3(1);
    modelMatrix *= transform2D::Translate(250, 500)
        * transform2D::Translate(translateX, translateY);
    RenderMesh2D(meshes["rectangle1"], shaders["VertexColor"], modelMatrix);

    modelMatrix = glm::mat3(1);
    modelMatrix *= transform2D::Translate(translateX, translateY)
        * transform2D::Translate(210, 350)
